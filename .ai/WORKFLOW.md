...
*Used only once when starting a new project.*
...
Step 0.2: Ideation (if PROJECT.md does not exist)
  ├── Use project_ideator approach — interview the human
  ...
  ├── Iteratively build PROJECT.md
  └── Present for human approval

Step 0.3: Project Initiation
  ├── Read approved PROJECT.md
  ├── Design system architecture → write ARCHITECTURE.md
  ├── Set up project structure (directories, config files)
  ├── Create TASKS.md with WBS (Work Breakdown Structure)
  ├── Create ROADMAP.md with milestones and timeline estimates
  ├── Create GLOSSARY.md with domain terminology
  ├── Create DECISIONS.md (empty, first decision recorded)
  ├── Create CONVENTIONS.md with initial coding conventions
